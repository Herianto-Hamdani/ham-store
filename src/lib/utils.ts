...
export function templateStyleVars(settings: TemplateInput): CSSProperties {
  return {
    ...
    ["--tpl-title-bottom" as string]: `${clamp(settings.templateTitleBottom, 2, 40)}%`,
    ...
    ["--tpl-photo-top" as string]: `${clamp(settings.templatePhotoTop, 4, 80)}%`,
    ["--tpl-photo-left" as string]: `${clamp(settings.templatePhotoLeft, 0, 60)}%`,
    ["--tpl-photo-width" as string]: `${clamp(settings.templatePhotoWidth, 20, 88)}%`,
    ["--tpl-photo-height" as string]: `${clamp(settings.templatePhotoHeight, 24, 84)}%`,
